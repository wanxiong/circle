 ...
 */
  customCompute?: (data: DrawPoint, other: {
    pageY: number;
...
export default class OutputPdf {
  ...
  constructor(options: OptionData) {
    ...
    if (!(element instanceof HTMLElement)) {
      throw new Error("Invalid argument: 'element' must be an instance of HTMLElement. Please ensure that the provided element is a valid HTML element.")
    }
    this.options = {
      ...options,
  ...
  addImage(imageData: ImageDataType,
    baseX = 0,
    y = 0,
    contentWidth: number,
  ...
  private async drawContent(imageData: ImageDataType, contentWidth: number, contentHeight: number, totalHeight: number) {
  ...
  isPureColor(data: Uint8ClampedArray, rgb?: number[]) {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      if (!themeColor) {
        ...
        if (debug) {
          console.log(`isPureColor (x轴canvas绘制像素点到 => ${i === 0 ? 1 : i / 4 + 1} px中止) (x对应html => 绘制像素点到${Math.ceil((i === 0 ? 1 : i / 4 + 1) / scale)}px中止) (主题色 => ${themeColor}) (中断色 => ${str})`)
        }
        // 非连续性颜色
        isPureColor = false;
        break;
  ...
  computeAdjustmentNum(data: DrawPoint & {
    canvas: HTMLCanvasElement;
    threshold?: number;
    // 指定色组
    rgb?: number[]
    ...
    while (!isPure) {
      ...
      if (typeof customCompute === "function") {
        const { resPointList: resultDataList, pureColorCondition = "and" } = customCompute({ x, y: sy, w, h }, { pageY, element: this.findElementByY(pageY - num) });
  ...
  printImg(imageData: string | ImageData | { imgData: ImageData, id: string }[], type: string, canvas?: HTMLCanvasElement) {
  ...
  async runExportPage() {
    let index = 1;
    let yPosition = 0;
    const { element, contentWidth, header: headerElement, scale, containerPadding = 0 } = this.options
    const pdf = this.pdf
    ...
    while (yPosition < renderedHeight) {
      ...
      // 添加标题
      this.drawTitle(index, this.tHeaderHeight, (h: number) => { this.tHeaderHeight += h })

      // 添加页眉
      if (headerElement) this.drawHeader(index, this.tHeaderHeight, (h: number) => { this.tHeaderHeight += h })
      ...
      if (diffCanvasHeight && !heightOutOfRange) {
        this.addBlank(0, this.baseY + this.tHeaderHeight + imageHeight - diffCanvasHeight, this.A4_WIDTH, diffCanvasHeight)
      }
      // 添加高度
  ...
  collectElementRanges(container: HTMLElement): ElementRange[] {
    ...
    Array.from(children).forEach((child) => {
      ...
      const endY = (rect.bottom - containerRect.top) * scale; // 相对于 container 的 bottom 坐标
